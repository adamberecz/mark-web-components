...
export default {
  ...
  setup(props, context)
  {
    ...
    // @added: deconstruct refs while keeping them reactive
    const { url, t } = toRefs(props)
    ...
    onMounted(async () => {
      ...
      alert('CreateListing.vue is mounted!'); // Visual indicator for debugging
      const data = (await axios.get('/sampleData/sampleData.json')).data

      // @added: you need to load data.data
      form.load(data.data)

      // @added: setting form submission endpoint
      vueform.value.endpoint = url

      // @added: set axios headers for each request
      // (this will be added to EACH axios requests)
...
<style>
.vf-create-listing *,
.vf-create-listing *:before,
.vf-create-listing *:after,
.vf-create-listing:root {
  --vf-primary: #07bf9b;
